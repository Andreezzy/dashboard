class ExpenseTypesController < ApplicationController
  ...
  def show
    #authorize! :read, @expense_types
    #authorize! :read, ExpenseType, :message => "Unable to read this article."
  end
  ...
  def create
    @expense_type = current_user.expense_type(expense_type_params)
  ...
  private
    ...
    def expense_type_params
      params.require(:expense_type).permit(:description)
    end
end
